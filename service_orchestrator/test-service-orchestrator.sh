...
# Fetch slice information for slice 1-100000 and capture the response
print_subheader "Fetching slice information for slice 1-100000"
slice_info=$(curl -s $BASE_URL/slices/1-100000)

# Check if the response contains valid data
if [ -n "$slice_info" ]; then
    print_success "Successfully retrieved slice information for slice 1-100000."
    print_info "Slice Information: $slice_info"
else
    print_error "Failed to fetch slice information for slice 1-100000."
    exit 1
fi